...
# Get current resource group from Learn Sandbox
RESOURCEGROUPSTRING=$(az group list --query "[0].name")
RESOURCEGROUPNAME=`sed -e 's/^"//' -e 's/"$//' <<<"$RESOURCEGROUPSTRING" `

LOCATIONSTRING=$(az group list --query "[0].location")
LOCATION=`sed -e 's/^"//' -e 's/"$//' <<<"$LOCATIONSTRING" `
...
#az monitor log-analytics workspace create --resource-group $RESOURCEGROUPNAME --location $LOCATION --workspace-name $RESOURCENAME --query-access disabled --ingestion-access disabled
...
# WORKS - az monitor app-insights component create --resource-group $RESOURCEGROUPNAME --location $LOCATION --app jimb-rentals-with-data-83422584 --workspace jimb-rentals-with-data-83422584
echo "Create app insights"
az monitor app-insights component create --resource-group $RESOURCEGROUPNAME --location $LOCATION --app $RESOURCENAME #--workspace $RESOURCENAME	
...
echo "Create web app"
az webapp up --resource-group $RESOURCEGROUPNAME --location $LOCATION --name $RESOURCENAME --os-type $OSTYPE --runtime $RUNTIME #--debug

# - Connect to App Service
echo "Connect web app to app insights"
az monitor app-insights component connect-webapp --resource-group $RESOURCEGROUPNAME --location $LOCATION --app $RESOURCENAME --web-app $RESOURCENAME